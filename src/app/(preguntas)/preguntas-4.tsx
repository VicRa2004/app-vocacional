...
import { PREGUNTAS_4 } from '@/data/preguntas-4';

const VistaPreguntas: React.FC = () => {
  const { obtenerResultados } = usePreguntasStore();

  const preguntas: Pregunta[] = PREGUNTAS_4;
  ...
  const siguiente = () => {
    ...
    router.replace('/(games)/trivia');
  };

  return (
    <ScrollView contentContainerStyle={styles.container}>
      <Text style={styles.header}>Cuestionario de Intereses 4</Text>
